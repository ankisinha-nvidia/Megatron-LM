...
class TestInferenceWandbLogging:
    ...
    @pytest.mark.internal
    def test_rank_activity_metrics_single_rank(self):
        """Test per-rank busy and GPU utilization metrics in single-rank mode."""
        dynamic_context = self._get_dynamic_context(logging_step_interval=10, metrics_writer=None)
        ...
        engine = DynamicInferenceEngine(controller=mock_controller, context=dynamic_context)
        engine._busy_time_since_log_s = 0.25
        engine._metrics_window_start_s = 100.0

        busy_pct = engine._compute_busy_pct(current_time_s=100.5)
        assert busy_pct == pytest.approx(50.0)
        assert engine._busy_time_since_log_s == 0.0
        assert engine._metrics_window_start_s == pytest.approx(100.5)

        metrics = engine._build_rank_activity_metrics(
            busy_pct=busy_pct,
            gpu_util_pct=42.0,
        )

        assert metrics[f'inference/rank_activity/busy_pct/rank_{engine.rank}'] == pytest.approx(
            50.0
        )
        assert metrics[
            f'inference/rank_activity/gpu_utilization_pct/rank_{engine.rank}'
        ] == pytest.approx(42.0)
        assert metrics['inference/rank_activity/busy_pct_mean'] == pytest.approx(50.0)
        assert metrics['inference/rank_activity/busy_pct_min'] == pytest.approx(50.0)
        assert metrics['inference/rank_activity/busy_pct_max'] == pytest.approx(50.0)
        assert metrics['inference/rank_activity/gpu_utilization_pct_mean'] == pytest.approx(
            42.0
        )
        assert metrics['inference/rank_activity/gpu_utilization_pct_min'] == pytest.approx(
            42.0
        )
        assert metrics['inference/rank_activity/gpu_utilization_pct_max'] == pytest.approx(
            42.0
        )
