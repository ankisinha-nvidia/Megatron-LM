    ...
    def _dynamic_inference(self, hidden_states: torch.Tensor, context: DynamicInferenceContext):
        ...
        if y_decode is not None and y_prefill is not None:
            ...
            if y_decode.shape[0] < token_count:
                # Padded prefill slots can exist even when no real prefill output
                # was produced. Pad decode output so the residual path sees the
                # expected token_count-sized tensor.
                y = torch.zeros(
